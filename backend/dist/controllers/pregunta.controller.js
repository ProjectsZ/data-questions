...
class PreguntaController {
    ...
    async createPregunta(req, res) {
        ...
        try {
            ...
            const categoria = await categoria_model_1.default.findById(pr_cat_id);
            if (!categoria) {
            ...
            const nuevaPregunta = new pregunta_model_1.default({
                pr_content,
                pr_img,
                pr_answer,
                pr_type,
        ...
        try {
            ...
            if (pr_answer && Array.isArray(pr_answer)) {
                ...
                for (let answer of pr_answer) {
                    const alternativa = await alternativa_model_1.default.findById(answer.pr_opt_id);
                    if (!alternativa) {
                        return res.status(400).json({
                            ok: false,
    ...
    async getPreguntas(req, res) {
        try {
            ...
            const [preguntas, totalPreguntas] = await Promise.all([
                pregunta_model_1.default.find() // Obtiene todas las preguntas
                ...
                pregunta_model_1.default.countDocuments() // Cuenta el total de documentos en la colección
            ]);
    ...
    async getPreguntaById(req, res) {
        const { pr_id } = req.params;
        try {
            const pregunta = await pregunta_model_1.default.findById(pr_id).populate('pr_cat_id', 'name'); // Populate para obtener la categoría
            if (!pregunta) {
    ...
    async getPreguntasByCategoryId(req, res) {
        const { pr_cat_id } = req.params;
        try {
            const preguntas = await pregunta_model_1.default.find({ pr_cat_id }).populate('pr_cat_id', 'name');
            return res.json({
                ok: true,
                preguntas
            });
    ...
    async getPreguntasByTag(req, res) {
        const { pr_tags } = req.params;
        try {
            const preguntas = await pregunta_model_1.default.find({ pr_tags: { $in: [pr_tags] } });
            return res.json({
                ok: true,
                preguntas
            });
    ...
    async updatePregunta(req, res) {
        ...
        const { pr_content, pr_img, pr_answer, pr_type, pr_difficulty, pr_time, pr_tags, pr_cat_id } = req.body;
        try {
            const pregunta = await pregunta_model_1.default.findById(pr_id);
            if (!pregunta) {
                return res.status(404).json({
                    ok: false,
                    msg: 'Pregunta no encontrada.'
    ...
    async deletePregunta(req, res) {
        ...
        try {
            ...
            await pregunta_model_1.default.findByIdAndDelete(pr_id);
            return res.json({
                ok: true,
    ...
    async getPreguntasByDifficulty(req, res) {
        const { pr_difficulty } = req.params;
        try {
            const preguntas = await pregunta_model_1.default.find({ pr_difficulty });
            return res.json({
                ok: true,
                preguntas
            });
    ...
    async getPreguntasByType(req, res) {
        const { pr_type } = req.params;
        try {
            const preguntas = await pregunta_model_1.default.find({ pr_type });
            return res.json({
                ok: true,
                preguntas
            });
    ...
    async getPreguntasByTimeLimit(req, res) {
        const { pr_time } = req.params;
        try {
            const preguntas = await pregunta_model_1.default.find({ pr_time: { $lte: pr_time } });
            return res.json({
                ok: true,
                preguntas
            });
    ...
    async getPreguntaAnswer(req, res) {
        const { pr_id } = req.params;
        try {
            const pregunta = await pregunta_model_1.default.findById(pr_id);
            if (!pregunta) {
